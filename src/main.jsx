...
function AddProduct({onAdd,onClose,all,cur,push}){
  ...
  const aiDetect=async()=>{
    if(!f.photo){push('Upload or take a product photo first!','warn');return;}
    setAiLoading(true);
    try{
      const prompt='Read this product label image. Reply with ONLY a JSON object, no other text, no markdown: {"name":"product name","company":"brand name","cat":"Grains or Spices or Drinks or Dairy or Snacks or Vegetables or Fruits or Cleaning or Personal Care or Other","unit":"size like 1kg or 500ml"}';
      ...
      push('AI detected: '+(result.name||'product'),'success');
    }catch(e){
      console.warn('AI detect:',e.message);
      push('AI could not read label — please fill in manually.','warn');
    }
    setAiLoading(false);
  };
  const goStep3=()=>{
