...
function SmartLens({products,cur,push}){
  ...
  // ── ANTHROPIC VISION AI ─────────────────────────────────────
  const analyzeWithAI = async (imgBase64, textQuery) => {
    try {
      const isImg = !!imgBase64;
      const messages = isImg ? [{
        role: 'user',
        content: [
          { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: imgBase64.split(',')[1] } },
          { type: 'text', text: `You are analyzing a product image for a grocery shop inventory system.
Identify the product in this image and respond ONLY with a JSON object (no markdown, no backticks):
{
  "name": "product name",
  "brand": "brand/company name or empty string",
  "category": "one of: Grains|Spices|Drinks|Dairy|Snacks|Vegetables|Fruits|Cleaning|Personal Care|Other",
  "unit": "packaging size e.g. 1kg or 500ml or empty",
  "confidence": 85,
  "description": "one sentence about this product",
  "search_terms": ["term1", "term2", "term3"]
}` }
        ]
      }] : [{
        role: 'user',
        content: `You are a grocery product search assistant for a shop inventory system.
For the product query: "${textQuery}"
Respond ONLY with a JSON object (no markdown, no backticks):
{
  "name": "most likely product name",
  "brand": "common brand or empty",
  "category": "one of: Grains|Spices|Drinks|Dairy|Snacks|Vegetables|Fruits|Cleaning|Personal Care|Other",
  "unit": "typical unit e.g. 1kg or 500ml",
  "confidence": 90,
  "description": "brief product description",
  "search_terms": ["${textQuery}", "similar term", "category term"]
}`
      }];
  ...
  // ── SEARCH INVENTORY ────────────────────────────────────────
  const searchInventory = (terms) => {
    const tl = terms.map(t=>t.toLowerCase());
    return products.filter(p => {
      const fields = [p.name, p.company, p.cat, p.unit].join(' ').toLowerCase();
      return tl.some(t => fields.includes(t));
    }).map(p => ({
      ...p,
      score: Math.min(99, tl.reduce((s,t) => s + (p.name.toLowerCase().includes(t)?35:p.company.toLowerCase().includes(t)?15:p.cat.toLowerCase().includes(t)?10:0), 60))
    })).sort((a,b) => b.score-a.score);
  };

  // ── MAIN SEARCH FLOW ────────────────────────────────────────
  const runSearch = useCallback(async (q, img) => {
    if(!q&&!img){push('Enter a product name or scan a photo','warn');return;}
    setPhase('analyzing');setAiResult(null);setLocalRes([]);setOnlineRes([]);setSel(null);

    // Step 1: AI analysis
    const ai = await analyzeWithAI(img||null, q||'');
    setAiResult(ai);

    // Step 2: Search inventory with AI terms
    setPhase('inventory');
    const terms = ai ? [ai.name, ai.brand, ...(ai.search_terms||[]), q].filter(Boolean) : [q];
    const found = searchInventory(terms);
    setLocalRes(found);

    // Step 3: Online simulation with AI-enriched results
    setPhase('online');
    await new Promise(r=>setTimeout(r,900));
    const baseKey = (ai?.name||q||'').toLowerCase();
    setOnlineRes(getOnline(baseKey));
    setPhase('done');

    if(found.length>0) push(`Found ${found.length} in inventory + online prices`,'success');
    else push('Not in inventory — showing online prices','info');
  },[products,push]);
  ...
  const handleImg = async e => {
    ...
    try{
      ...
      setQuery('');
      push('Photo loaded — AI analyzing...','info');
      runSearch('', compressed);
    } catch{push('Failed to load image','error');}
    e.target.value='';
  };

  const camCapture = url => {
    setShowCam(false); setImgData(url); setQuery('');
    push('Photo captured — AI analyzing...','info');
    runSearch('', url);
  };
    ...
    <div style={{padding:'20px 16px 100px'}}>
      ...
      <div className="pp" style={{background:T.surf,border:`1.5px solid ${T.P}28`,borderRadius:20,padding:20,boxShadow:T.shadowM}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
          {[[fm(cv(sel.buy,sel.cur,cur),cur),'Buy Price',T.P],[fm(cv(sel.sell,sel.cur,cur),cur),'Sell Price',T.IN],
            [sel.cat,'Category',T.A],[`+${(((sel.sell-sel.buy)/sel.buy)*100).toFixed(1)}%`,'Margin',T.ok]].map(([v,l,c])=>(
            <div key={l} style={{background:T.bg2,borderRadius:12,padding:'10px 12px',border:`1.5px solid ${c}18`}}>
        ...
        <div style={{background:`${T.ok}0e`,border:`1px solid ${T.ok}28`,borderRadius:10,padding:'9px 12px',marginTop:12,fontSize:12,color:T.ok}}>
          ✓ Found in inventory · Stock: {sel.qty} units
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{padding:'20px 16px 100px'}}>
      <div style={{marginBottom:16}}>
        ...
        <p style={{color:T.ink3,fontSize:13}}>AI-powered visual search — like Google Lens for your shop</p>
      </div>

      {/* Mode toggle */}
      <div style={{display:'flex',background:T.bg2,borderRadius:13,padding:4,marginBottom:16,border:`1px solid ${T.bdr}`}}>
        {[{id:'text',icon:'🔍',label:'Text'},{id:'photo',icon:'📸',label:'Photo / Camera'}].map(m=>(
          <div key={m.id} onClick={()=>setMode(m.id)} style={{flex:1,padding:'9px',borderRadius:10,
            textAlign:'center',cursor:'pointer',transition:'all .17s',display:'flex',alignItems:'center',
            justifyContent:'center',gap:6,background:mode===m.id?T.surf:'transparent',
            color:mode===m.id?T.ink:T.ink3,fontWeight:mode===m.id?700:500,fontSize:13,
            boxShadow:mode===m.id?T.shadow:'none'}}>
        ...
        <div style={{display:'flex',gap:10,marginBottom:14}}>
          <div style={{flex:1,position:'relative'}}>
            ...
            <input value={query} onChange={e=>setQuery(e.target.value)}
              onKeyDown={e=>e.key==='Enter'&&runSearch(query,null)}
              ...
              style={{width:'100%',background:T.inputBg,border:`1.5px solid ${T.bdr}`,borderRadius:13,
                padding:'12px 14px 12px 40px',color:T.ink,fontSize:13,outline:'none',fontFamily:BODY,
                caretColor:T.P,transition:'border-color .17s'}}
              onFocus={e=>e.target.style.borderColor=`${T.P}77`}
              onBlur={e=>e.target.style.borderColor=T.bdr}/>
          </div>
          <Btn onClick={()=>runSearch(query,null)} sx={{padding:'12px 16px',borderRadius:13,flexShrink:0}}>Search</Btn>
        </div>
      )}
      ...
      {mode==='photo'&&(
        <div style={{marginBottom:14}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
            <div onClick={()=>setShowCam(true)} style={{background:`${T.P}08`,border:`1.5px dashed ${T.P}44`,
              borderRadius:16,padding:'20px',textAlign:'center',cursor:'pointer',transition:'all .18s'}}
              ...
              <div style={{fontWeight:700,color:T.P,fontSize:12,fontFamily:HEAD}}>Open Camera</div>
              <div style={{fontSize:10,color:T.ink4,marginTop:2}}>Point at any product</div>
            </div>
            <div>
              <input ref={fileRef} type="file" accept="image/*" onChange={handleImg} style={{display:'none'}} id="lens_file"/>
              <label htmlFor="lens_file" style={{display:'flex',flexDirection:'column',alignItems:'center',
                justifyContent:'center',minHeight:120,background:`${T.IN}08`,border:`1.5px dashed ${T.IN}44`,
                ...
                <div style={{fontWeight:700,color:T.IN,fontSize:12,fontFamily:HEAD}}>Upload Photo</div>
          ...
          {imgData&&(
            <div style={{background:T.bg2,borderRadius:13,padding:10,display:'flex',gap:10,alignItems:'center',border:`1px solid ${T.bdr}`}}>
              <img src={imgData} alt="" style={{width:52,height:52,borderRadius:10,objectFit:'cover',border:`1px solid ${T.bdr}`}}/>
              <div style={{flex:1}}>
                <div style={{fontWeight:600,color:T.ink,fontSize:13}}>{aiResult?aiResult.name:'AI analyzing...'}</div>
                <div style={{fontSize:11,color:T.ink4,marginTop:1}}>{aiResult?aiResult.description||aiResult.category:'Processing image with AI'}</div>
              </div>
              {phase==='done'&&<Btn sm onClick={()=>runSearch(query,imgData)}>Re-scan</Btn>}
            </div>
          )}
        </div>
      )}
      ...
      {aiResult&&phase!=='analyzing'&&(
        <div style={{background:`linear-gradient(135deg,${T.P}12,${T.P}06)`,border:`1.5px solid ${T.P}28`,
          borderRadius:16,padding:'13px 15px',marginBottom:14,display:'flex',gap:12,alignItems:'center'}}>
          <div style={{width:40,height:40,borderRadius:12,background:T.PG,display:'flex',alignItems:'center',
            justifyContent:'center',fontSize:20,flexShrink:0,color:'#fff'}}>🤖</div>
          <div style={{flex:1}}>
            <div style={{fontWeight:700,color:T.ink,fontSize:13}}>AI identified: <span style={{color:T.P}}>{aiResult.name}</span>
              {aiResult.brand&&<span style={{color:T.ink3}}> by {aiResult.brand}</span>}
            </div>
            <div style={{fontSize:11,color:T.ink3,marginTop:2}}>{aiResult.category} · {aiResult.confidence||90}% confidence</div>
          </div>
        </div>
      )}

      {/* Loading phases */}
      {(phase==='analyzing'||phase==='inventory'||phase==='online')&&(
          ...
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            ...
            <div>
              <div style={{fontWeight:700,color:T.ink,fontSize:13}}>
                {phase==='analyzing'?'AI analyzing image...':phase==='inventory'?'Searching inventory...':'Finding online prices...'}
              </div>
              <div style={{fontSize:11,color:T.ink3,marginTop:1}}>
                {phase==='analyzing'?'Using Claude Vision AI to identify product'
                 :phase==='inventory'?`Checking ${products.length} products for matches`
                 :'Comparing prices across stores'}
              </div>
            </div>
          </div>
          {/* Progress bar */}
          <div style={{marginTop:12,height:3,background:T.bdr,borderRadius:3,overflow:'hidden'}}>
            <div style={{height:'100%',background:T.PG,borderRadius:3,
              width:phase==='analyzing'?'33%':phase==='inventory'?'66%':'90%',transition:'width .5s'}}/>
          </div>
        </div>
      )}
      ...
      {(phase==='online'||phase==='done')&&(
        <div style={{marginBottom:14}}>
          ...
          {localRes.length===0
            ?<div style={{background:T.bg2,borderRadius:13,padding:14,textAlign:'center',border:`1px solid ${T.bdr}`,marginBottom:4}}>
              <div style={{fontSize:26,marginBottom:4}}>📭</div>
              <div style={{fontSize:13,color:T.ink3,fontWeight:500}}>Not found in your inventory</div>
              <div style={{fontSize:11,color:T.ink4,marginTop:2}}>Check online results below</div>
            </div>
            :localRes.map(p=>(
              <div key={p.id} onClick={()=>setSel(p)} style={{background:T.surf,borderRadius:14,
                padding:'12px 13px',marginBottom:7,display:'flex',alignItems:'center',gap:12,cursor:'pointer',
                ...
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4}}>
                  ...
                  <div style={{background:`${T.ok}14`,color:T.ok,border:`1px solid ${T.ok}28`,
                    borderRadius:99,padding:'2px 8px',fontSize:10,fontWeight:700}}>{p.score}% match</div>
                </div>
              </div>
            ))
          }
      ...
      {phase==='done'&&(
        <div>
          <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:10}}>
            ...
            <div style={{marginLeft:'auto',fontSize:9,color:T.ink4,background:T.bg2,padding:'2px 8px',
              borderRadius:99,border:`1px solid ${T.bdr}`,fontWeight:600}}>AI POWERED</div>
          </div>
          {onlineRes.map((r,i)=>(
            <div key={i} style={{background:T.surf,borderRadius:14,padding:'12px 13px',marginBottom:7,
              ...
              <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4}}>
                ...
                <div style={{background:`${T.IN}14`,color:T.IN,border:`1px solid ${T.IN}28`,
                  borderRadius:99,padding:'2px 8px',fontSize:10,fontWeight:700}}>{r.m}% match</div>
              </div>
            </div>
          ))}
          <div style={{background:T.bg2,borderRadius:11,padding:'9px 13px',marginTop:4,
            fontSize:11,color:T.ink4,lineHeight:1.6,border:`1px solid ${T.bdr}`}}>
            Online prices are AI-estimated reference data. Actual prices vary by store.
          </div>
        </div>
      )}
      ...
      {phase==='idle'&&(
        <div style={{textAlign:'center',padding:'32px 20px',color:T.ink4}}>
          ...
          <div style={{fontWeight:800,fontSize:15,fontFamily:HEAD,color:T.ink3,marginBottom:6}}>AI-Powered Product Search</div>
          <div style={{fontSize:13,color:T.ink4,lineHeight:1.8,maxWidth:280,margin:'0 auto 20px'}}>
            Works like Google Lens — point your camera at any product and AI will identify it, search your inventory, and compare prices online
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:8,maxWidth:260,margin:'0 auto'}}>
            {['🤖 AI identifies any product instantly','📦 Searches your inventory first','💰 Compares online prices','📊 Shows match percentage'].map(f=>(
              <div key={f} style={{background:T.bg2,borderRadius:10,padding:'9px 14px',
                border:`1px solid ${T.bdr}`,fontSize:12,color:T.ink3,textAlign:'left',fontWeight:500}}>{f}</div>
            ))}
          </div>
        </div>
      )}
