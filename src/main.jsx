...
function AddProduct({onAdd,onClose,all,cur,push,editItem=null}){
  ...
  const goStep3=()=>{
    if(!isEdit){
      const exact=all.find(p=>p.name.toLowerCase().trim()===f.name.toLowerCase().trim()&&
        String(p.base)===f.base&&String(p.sell)===f.sell);
      if(exact){setRestockOf({...exact,isDuplicate:true});return;}
      const nameMatch=all.find(p=>p.id!==editItem?.id&&p.name.toLowerCase().trim()===f.name.toLowerCase().trim());
      if(nameMatch){setRestockOf(nameMatch);return;}
    }
    setStep(3);
  };
...
// ─── INVENTORY ───────────────────────────────────────────────────
function Inventory({products,cur,onDel,onEdit,push}){
  const T=useT();
  const [cat,setCat]=useState('All');
  const [q,setQ]=useState('');
  const [sel,setSel]=useState(null);
  const [sortBy,setSortBy]=useState('name'); // name|price|expiry|stock
  const [showSort,setShowSort]=useState(false);

  // Build all cats including any custom ones from products
  const allCats=useMemo(()=>{
    const custom=products.map(p=>p.cat).filter(c=>c&&!CATS.includes(c));
    return [...CATS,...new Set(custom)];
  },[products]);
  ...
  const fl=useMemo(()=>{
    ...
    if(sortBy==='price') list=[...list].sort((a,b)=>cv(b.buy,b.cur||'BDT',cur)-cv(a.buy,a.cur||'BDT',cur));
    else if(sortBy==='expiry') list=[...list].sort((a,b)=>new Date(a.expire||'9999')-new Date(b.expire||'9999'));
    else if(sortBy==='stock') list=[...list].sort((a,b)=>(a.qty||0)-(b.qty||0));
    else list=[...list].sort((a,b)=>a.name.localeCompare(b.name));
    return list;
  },[products,cat,q,sortBy,cur]);

  // Group by category for display
  const grouped=useMemo(()=>{
    if(cat!=='All') return null;
    const g={};
    fl.forEach(p=>{const c=p.cat||'Other';if(!g[c])g[c]=[];g[c].push(p);});
    return g;
  },[fl,cat]);
    ...
    <div style={{paddingBottom:100}}>
      <div style={{padding:'20px 16px 0'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
          ...
          <div style={{position:'relative'}}>
            <button onClick={()=>setShowSort(!showSort)} style={{background:T.bg2,border:`1.5px solid ${T.bdr}`,borderRadius:10,padding:'7px 12px',
              cursor:'pointer',fontSize:12,fontWeight:600,color:T.ink3,display:'flex',alignItems:'center',gap:5,fontFamily:BODY}}>
              ⇅ {sortBy==='name'?'A-Z':sortBy==='price'?'Price':sortBy==='expiry'?'Expiry':'Stock'}
            </button>
            {showSort&&(
              <div className="pp" style={{position:'absolute',right:0,top:'110%',background:T.surf,border:`1.5px solid ${T.bdr}`,
                borderRadius:13,padding:6,zIndex:100,boxShadow:T.shadowM,minWidth:140}}>
                {[['name','Name A–Z'],['price','Price High–Low'],['expiry','Expiry Soonest'],['stock','Stock Low–High']].map(([k,v])=>(
                  <div key={k} onClick={()=>{setSortBy(k);setShowSort(false);}} style={{padding:'9px 13px',borderRadius:9,cursor:'pointer',
                    fontSize:12,fontWeight:600,color:sortBy===k?T.P:T.ink3,background:sortBy===k?`${T.P}0d`:'transparent',transition:'all .15s'}}
                    onMouseOver={e=>e.currentTarget.style.background=sortBy===k?`${T.P}0d`:T.bg2}
                    onMouseOut={e=>e.currentTarget.style.background=sortBy===k?`${T.P}0d`:'transparent'}>{v}</div>
                ))}
              </div>
            )}
          </div>
        ...
        <div style={{display:'flex',gap:7,overflowX:'auto',paddingBottom:3}}>
          {allCats.map(c=><div key={c} onClick={()=>setCat(c)}
            style={{padding:'6px 14px',borderRadius:99,fontSize:11,fontWeight:700,whiteSpace:'nowrap',cursor:'pointer',flexShrink:0,transition:'all .15s',
              background:cat===c?T.P:T.surf,color:cat===c?'#fff':T.ink3,border:`1.5px solid ${cat===c?T.P:T.bdr}`}}>
            {CE[c]||'📦'} {c}</div>)}
        </div>
      </div>
      ...
      <div style={{padding:'12px 16px'}}>
        ...
        {fl.length===0?(
          <div style={{textAlign:'center',padding:'60px 20px',color:T.ink4}}>
            ...
            <div style={{marginTop:10,fontWeight:600,fontSize:14,fontFamily:HEAD,color:T.ink3}}>Nothing found</div>
          </div>
        ):cat==='All'&&grouped&&q===''?(
          // Grouped by category when browsing all
          Object.keys(grouped).sort().map(catName=>(
            <div key={catName} style={{marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:T.P,letterSpacing:.8,textTransform:'uppercase',
                marginBottom:8,display:'flex',alignItems:'center',gap:6}}>
                <span>{CE[catName]||'📦'}</span> {catName} <span style={{color:T.ink4,fontWeight:600}}>({grouped[catName].length})</span>
              </div>
              {grouped[catName].map(p=><PCard key={p.id} p={p} cur={cur} onPress={()=>setSel(p)}
                onTrash={id=>{onDel(id);push('Moved to trash','info');}}/>)}
            </div>
          ))
        ):(
          fl.map(p=><PCard key={p.id} p={p} cur={cur} onPress={()=>setSel(p)}
            onTrash={id=>{onDel(id);push('Moved to trash','info');}}/>)
        )}
      </div>
      ...
      {sel&&(
        <Sheet title={sel.name} sub={`${sel.company||'—'} · ${sel.unit||'—'} · ${sel.cat}`} onClose={()=>setSel(null)}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:14}}>
            {[[fm(cv(sel.buy,sel.cur||'BDT',cur),cur),'Buy',T.P],
              [fm(cv(sel.sell,sel.cur||'BDT',cur),cur),'Sell',T.IN],
          ...
          <div style={{background:T.bg2,borderRadius:14,border:`1.5px solid ${T.bdr}`,overflow:'hidden',marginBottom:14}}>
            {[['Base',fm(cv(sel.base,sel.cur||'BDT',cur),cur)],
          ...
          {/* Actions */}
          <div style={{display:'flex',gap:10}}>
            <Btn v="ghost" onClick={()=>setSel(null)} sx={{flex:1}}>Close</Btn>
            <Btn onClick={()=>{setSel(null);onEdit(sel);}} sx={{flex:2,fontFamily:HEAD}}>✏️ Edit Product</Btn>
          </div>
        </Sheet>
      )}
    </div>
    ...
    <ThemeCtx.Provider value={T}>
      <div style={{fontFamily:BODY,background:T.bg,minHeight:'100vh',color:T.ink,maxWidth:480,margin:'0 auto',position:'relative'}}>
        ...
        <div style={{position:'relative',zIndex:1}}>
          ...
          {page==='inv'&&<Inventory products={prods||[]} cur={cur} onDel={moveToTrash} onEdit={p=>{setEditItem(p);setShowAdd(true);}} push={push}/>}
